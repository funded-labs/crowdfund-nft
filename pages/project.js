...
export default function ProjectDetails() {
  ...
  const { data: project, isLoading, isError, isFetching } = useProjectDetails(projectId);
  ...
  return (
    <div className="w-full bg-gray-50">
      ...
      {selectedTab === "campaign-details" && <CampaignDetails project={project} author={project.owner} />}
      {selectedTab === "nft-collection" && <NFTCollection project={project} />}
      {selectedTab === "faqs" && <Faqs project={project} />}

      {selectedTab === "activity" && <Activity project={project} escrowActor={project.escrowActor} />}
