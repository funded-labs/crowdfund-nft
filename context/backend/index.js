...
const NFID_APPLICATION_LOGO_URL =
  'https%3A%2F%2Fgateway.pinata.cloud%2Fipfs%2FQmcZjaQLaKwxtKGKaneSpNqpre863wTUU5jyVZAE3XG4uw'
...
export function BackendProvider({
  ...
  const getPlugPrincipal = async () => {
    ...
    if (!window?.ic?.plug) return alert('Plug is not installed in your browser')
    if (
  ...
  const authClientLogin = async (provider) => {
    ...
    return new Promise((resolve, reject) => {
      authClient.login({
        identityProvider,
        derivationOrigin: window.location.origin.includes('crowdfund-nft.com')
