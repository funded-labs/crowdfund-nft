...
const useMessityMessaging = () => {
  ...
  useEffect(() => {
    ...
    messityClient.getSentMessages().then(setSentMessages).catch(console.log)
  ...
  const loadConversation = useCallback(
    (withPrincipal) => {
      ...
      return formatConversation(
        sentMessages.filter(
          (message) => message.header.to[0].toText() === withPrincipal,
        ),
        receivedMessages.filter(
          (message) => message.header.sender.toText() === withPrincipal,
        ),
      )
    },
